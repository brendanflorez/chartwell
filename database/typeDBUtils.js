...
const connectToDatabase = async () => {
  ...
  const session = await client.session("chartwell_02", SessionType.DATA);
...
for (const duplicateIID of iids) {
  ...
  const updateCausalityRelationships = `
    match
      $relation (cause: $duplicate, effect: $other) isa causality, has confidence $confidence, has direction $direction;
      $main iid ${mainConceptIID};
      $duplicate iid ${duplicateIID};
    insert
      $new_relation (cause: $main, effect: $other) isa causality, has confidence $confidence, has direction $direction;
    delete $relation isa causality;
  `;
  await transaction.query(updateCausalityRelationships);
  ...
  const updateSimilarityRelationships = `
    match
      $relation (concept-1: $duplicate, concept-2: $other) isa similarity, has score $score;
      $main iid ${mainConceptIID};
      $duplicate iid ${duplicateIID};
    insert
      $new_relation (concept-1: $main, concept-2: $other) isa similarity, has score $score;
    delete $relation isa similarity;
  `;
  await transaction.query(updateSimilarityRelationships);

  // Update the similarity relationships where the non-main concept is concept-2
  const updateSimilarityRelationshipsInverse = `
    match
      $relation (concept-1: $other, concept-2: $duplicate) isa similarity, has score $score;
      $main iid ${mainConceptIID};
      $duplicate iid ${duplicateIID};
    insert
      $new_relation (concept-1: $other, concept-2: $main) isa similarity, has score $score;
    delete $relation isa similarity;
  `;
  await transaction.query(updateSimilarityRelationshipsInverse);
